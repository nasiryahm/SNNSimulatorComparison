...
fig, ax = plt.subplots()
ax.set_title("Vogels-Abbott Benchmark Result with 0.1ms delay")
ax.bar(
...
ax.set_ylabel("Simulation Speed Relative to Real-time")
ax.set_yscale('log')
...
fig, ax = plt.subplots()
ax.set_title("Vogels-Abbott Benchmark Result with 0.8ms delay")
ax.bar(
        range(len(simulators)),
        eight_timestep_delay_results,
        tick_label=simulators,
